...
| Explorable local incident map | Full-screen Mapbox GL with drag, pinch zoom, current position, received mesh reports, and recenter | Built; physical-device render verified; no synthetic runtime markers |
| Bluetooth relay remains active regardless of cloud state | Native launch, foreground, and reachability observers | Built; physical revalidation pending |
| Offline OpenStreetMap tiles | Cache Storage-backed local tile region | Built; bounded 25-tile region |
| Automatic cloud upload, duplicate receipts, and sync attempt ledger | JacHammer function endpoints and durable evidence ledgers | Built; local HTTP path verified; public JacHammer production URL pending |
...
- A durable receipt from the deployed JacHammer endpoint on a physical iPhone.
